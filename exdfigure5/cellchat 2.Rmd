...
```{r}
setwd("E:/reshif1 snseq/differential cell chat")
```

```{r}
cellchat.wt <- readRDS("E:/reshif1 snseq/cellchat/cellchat_wt.rds")
cellchat.ko <- readRDS("E:/reshif1 snseq/cellchat/cellchat_ko.rds")
```
...
```{r}
png(filename="./signaling changes res.png", width=75, height=20, units="cm", res=300)
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "resident_mac")
gg1
dev.off()
```
...
```{r}
png(filename="./DiffProb ligand receptor pairs.png", width=30, height=30, units="cm", res=300)
netVisual_bubble(cellchat, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibrobalst', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocte', 'smc', 'pdc', 'neuron'),  comparison = c(1, 2), angle.x = 45)
dev.off()
```

```{r}
gg1 <- netVisual_bubble(cellchat, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibrobalst', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocte', 'smc', 'pdc', 'neuron'),  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in KO", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibrobalst', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocte', 'smc', 'pdc', 'neuron'),  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in KO", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object

png(filename="./DiffProb ligand receptor pairs split.png", width=30, height=30, units="cm", res=300)
gg1 + gg2
dev.off()
```
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibrobalst', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocte', 'smc', 'pdc', 'neuron'), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibrobalst', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocte', 'smc', 'pdc', 'neuron'), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
png(filename="./DEX ligand receptor pairs split.png", width=30, height=30, units="cm", res=300)
gg1 + gg2
dev.off()
```
...
netVisual_chord_gene(object.list[[2]], sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibroblast', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocyte', 'smc', 'pdc', 'neuron'), slot.name = 'net', net = net.up, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
dev.off()

png(filename="./chord diagram signaling down ko.png", width=30, height=15, units="cm", res=300)
netVisual_chord_gene(object.list[[1]], sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibroblast', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocyte', 'smc', 'pdc', 'neuron'), slot.name = 'net', net = net.down, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> You may try the function `netVisual_chord_cell` for visualizing individual signaling pathway
dev.off()
```

```{r}
png(filename="./chord diagram gas.png", width=30, height=15, units="cm", res=300)
pathways.show <- c("GAS") 
weight.max <- getMaxWeight(object.list, slot.name = c("netP"), attribute = pathways.show) # control the edge weights across different datasets
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_aggregate(object.list[[i]], signaling = pathways.show, sources.use = "resident_mac", targets.use = c('myeloid', 'neutrophil', 'fibroblast', 'endocardial', 'endothelial', 'pericyte', 'epicardium', 'nkt', 'cardiomyocyte', 'lymphatic', 'bcell', 'adipocyte', 'smc', 'pdc', 'neuron'), layout = "circle", edge.weight.max = weight.max[1], edge.width.max = 10, signaling.name = paste(pathways.show, names(object.list)[i]))
}
dev.off()
```
